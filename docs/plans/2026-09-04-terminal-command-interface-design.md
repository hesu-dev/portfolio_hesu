...
터미널은 transcript와 입력 행을 하나의 스크롤 흐름으로 구성한다. 입력 행은 화면 하단에 고정하지 않고 마지막 출력 바로 다음 줄에 붙인다. 첫 화면에서는 `help` 안내가 시작점에 있고, 그 안내 목록 직후에 `포트폴리오: ~$` 프롬프트와 실제 `TextField`의 점멸 커서가 보인다. placeholder, 전송 아이콘, 입력 박스 외곽선, 별도 배경, hover·selection 강조는 사용하지 않는다. Enter 또는 모바일 키보드의 전송 액션으로 실행하며, 터미널을 열거나 명령을 실행·초기화하거나 터미널 본문을 다시 누르면 입력란으로 포커스를 돌려 점멸 커서를 유지한다.
