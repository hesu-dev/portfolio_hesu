...
| 모바일 Skills 상단에 자격증 카드 배치 | 충족 | compact 화면 상단 카드 영역에 `정보처리기사` 자격증 카드 1개를 표시한다. |
| 모바일 Skills 채널 수를 데스크탑과 통일 | 충족 | 자격증 카드 아래에 Development, Collaboration, Design & UI/UX의 3개 그룹만 `#` 채널로 표시한다. |
| 모바일 Skills를 채널 목록·상세 2뎁스로 구성 | 충족 | 채널 선택 시 고정 `#채널 · 1명의 멤버 · 1개의 탭` 헤더와 날짜·프로필·이름·시간·활동 본문을 가진 메시지 피드로 이동한다. |
| 모든 폼팩터의 Skills 채널 내부를 공통화 | 충족 | 데스크탑·태블릿·모바일이 공통 헤더·날짜·프로필·이름·시간·활동 메시지 컴포넌트를 사용하고 탐색 구조만 다르게 유지한다. |
