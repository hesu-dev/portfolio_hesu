...
| 초기 `help` 명령과 명령·설명 2열 안내 | 충족 | 첫 transcript를 `포트폴리오: ~$ help`로 시작하고 영문 명령 7개와 한국어 설명을 구조화된 2열로 표시한다. 200% 글자 크기의 좁은 화면에서는 같은 정보를 세로로 안전하게 재배치한다. |
...
| `git log`, `git status`, `ls`를 실제 포트폴리오 내용에 연결 | 충족 | `git log`는 실제 최근 커밋의 배포 스냅샷을 최신순으로, `git status`는 `dev`의 clean 상태를, `ls`는 공용 데이터의 개인 프로젝트만 출력한다. |
| `clear`로 터미널 완전 초기화 | 충족 | 초기 help·명령 echo·출력·입력값을 모두 지우고 스크롤을 처음으로 돌린 뒤 입력 포커스를 유지해 바로 다음 명령을 받을 수 있다. |
