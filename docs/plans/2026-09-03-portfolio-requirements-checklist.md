...
| 신호등 빨강 X·노랑 −·초록 무표식, 눌림 효과 제거 | 충족 | 공용 `MacTrafficControls`가 세 창 버튼을 한 번만 그리고 빨강/노랑에만 지정 글리프를 표시한다. hover·pressed 색상 변화는 없다. |
...
| 데스크탑 위치는 홈과 같은 앱 아이콘 목록 | 충족 | `portfolioLauncherAppIds`와 `AppleAppIcon`을 그대로 재사용하며 iPhone/iPad는 Finder를 보존한 창 스택 위에 앱을 열고, macOS는 독립 창으로 연다. |
...
| iPad/iPhone Finder 하단 탐색은 최근 항목·경력·개인만 제공 | 충족 | 내부 3탭 Finder 독이 각 위치 페이지에 연결되고, `개인 프로젝트`는 좁은 폭에서 자연스럽게 두 줄 처리된다. 홈 화면의 앱 Dock과는 별개다. |
...
| Finder 데스크탑의 앱은 기존 창을 교체하지 않고 새 창으로 열기 | 충족 | iPhone·iPad는 Projects를 창 스택에 보존한 채 선택 앱을 위에 열고 닫으면 `데스크탑` Finder 상태로 돌아간다. macOS도 각 앱을 독립 창으로 연다. 배경 창의 포인터·접근성·키보드 동작은 비활성화한다. |
...
| iPad/iPhone 설정은 기기 비율의 Light/Dark 미리보기 | 충족 | compact 설정은 프로필 아래에 세로형 화면 미리보기 두 개를 나란히 놓고 선택 상태를 즉시 반영한다. 옵션 아래의 불필요한 시스템 설정 항목은 모사하지 않는다. |
...
| 전체 정적 분석·테스트·Pages/Vercel 빌드 | 진행 중 | 최종 테마 보완 뒤 전체 테스트·정적 분석·두 base-href release build를 다시 실행한다. |
| 데스크톱·iPad·iPhone 실화면 검수 | 진행 중 | 최신 테마 보완 뒤 세 폼팩터와 양 테마를 로컬 인앱 브라우저에서 다시 확인한다. |

## 2026-09-03 최종 검증 기록

최신 메뉴 막대·시스템 패널 테마 보완 후 명령과 실화면 결과를 이 절에 갱신한다.
